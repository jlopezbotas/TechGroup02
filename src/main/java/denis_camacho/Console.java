...
public class Console {
    ...
    public Console(){
        public Console(Player player1, Player player2) {
            ...
            switch (option) {
                case "1":
                    ...
                    while (!game.isEndGame()) {
                        game.startGame();
                        while (!selectFieldFile(game.getTable(), player1, options)) {
                        }
                        if (game.getTable().verifyWinner(player1)) {
                            player1.updateFlagWinner();
                            System.out.println(String.format("Player %s winner %s of 3", player1.getName(), player1.getFlagWinner()));
                            game.getTable().cleaning();
                        }
                        if (game.getTable().fullTable()) {
                            game.getTable().cleaning();
                        }
                        if (player1.getFlagWinner() != 2) {
                            while (!selectFieldFile(game.getTable(), player2, options)) {
                            }
                            if (game.getTable().verifyWinner(player2)) {
    ...
    private void printer(Table table) {
    ...
    private boolean selectFieldFile(Table table, Player player, Scanner options) {
        printer(table);
